...
for (let day of days) {
    ...
    if (mon == d[0] && date == d[1]) {
        ...
        $("#change1").html("今天是中国国家纪念日，已切换为黑白模式");
        window.addEventListener('load', function () {
            setTimeout(function () {
                iziToast.show({
                    timeout: 14000,
...
$('#switchmore').on('click', function () {
    ...
    if (shoemore && $(document).width() >= 990) {
        ...
        $("#change1").html("时间不在于你拥有多少，而在于你怎样使用。——艾克");
    } else {
        $('#container').attr('class', 'container');
        $("#change").html("Welcome&nbsp;to my website!&nbsp;!");
